...
struct SignalRegistry {
  absl::Mutex mu;
  absl::flat_hash_map<std::pair<uint64_t, uint64_t>, xla::Promise<>> waiters
      ABSL_GUARDED_BY(mu);
  absl::flat_hash_set<std::pair<uint64_t, uint64_t>> signaled
      ABSL_GUARDED_BY(mu);
};
...
PJRT_Error* PJRT_Rma_ExportWindow(PJRT_Rma_ExportWindow_Args* args) {
  ...
  if (args->buffer->vtable != nullptr &&
      args->buffer->vtable->inc_ref != nullptr) {
    args->buffer->vtable->inc_ref(args->buffer);
  }
...
PJRT_Error* PJRT_Rma_DestroyRemoteWindow(
  ...
  if (args->remote_window) {
    auto& registry = GetWindowRegistry();
    absl::MutexLock lock(&registry.mu);
    registry.windows.erase(args->remote_window->window_id);
    if (args->remote_window->local_buffer) {
      registry.buffer_to_id.erase(args->remote_window->local_buffer);
...
PJRT_Error* PJRT_Rma_Signal(PJRT_Rma_Signal_Args* args) {
  ...
  if (it != registry.waiters.end()) {
    it->second.Set(absl::OkStatus());
    registry.waiters.erase(it);
  } else {
    registry.signaled.insert(key);
  }
  args->event = new PJRT_Event{xla::Future<>(absl::OkStatus())};
  return nullptr;
}
...
PJRT_Error* PJRT_Rma_WaitSignal(PJRT_Rma_WaitSignal_Args* args) {
  ...
  if (!args->local_buffer) {
    return StatusToPjRtError(
        absl::InvalidArgumentError("Null local_buffer in Rma_WaitSignal"));
  }
  ...
  auto& registry = GetSignalRegistry();
  absl::MutexLock lock(&registry.mu);
  auto sig_it = registry.signaled.find(key);
  if (sig_it != registry.signaled.end()) {
    registry.signaled.erase(sig_it);
    args->event = new PJRT_Event{xla::Future<>(absl::OkStatus())};
  } else {
    auto [promise, future] = xla::MakePromise();
    registry.waiters[key] = std::move(promise);
    args->event = new PJRT_Event{std::move(future)};
  }
  return nullptr;
}
