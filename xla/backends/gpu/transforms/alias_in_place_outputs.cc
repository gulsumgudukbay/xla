...
bool HasIdentityIndexing(const HloFusionInstruction& fusion, int64_t operand) {
  ...
  auto it = grouped.find(fusion.operand(operand));
  if (it == grouped.end()) {
    return false;
  }
  return it->second.size() == 1 &&
         it->second.begin()->map().GetSymbolicMap().IsIdentity();
}
...
bool AliasFusion(HloFusionInstruction& fusion,
  ...
  for (int64_t i = 0; i < fusion.operand_count(); ++i) {
    ...
    if (!HasIdentityIndexing(fusion, i)) {
      continue;
    }
  ...
  for (HloComputation* computation :
    ...
    for (HloInstruction* instr : computation->instructions()) {
      if (auto* fusion = DynCast<HloFusionInstruction>(instr);
        ...
        changed |= AliasFusion(*fusion, reachability_of());
