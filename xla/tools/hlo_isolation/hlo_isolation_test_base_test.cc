...
TEST_F(HloIsolationTest, TestDumpHloOutputCallbacks_WithMutator) {
  const char* hlo_text = R"(
HloModule TestModule
...
TEST_F(HloIsolationTest, TestDumpHloOutputCallbacks_NullAndEmptyLiterals) {
  const char* hlo_text = R"(
HloModule TestModule
...
TEST_F(HloIsolationTest, TestComparisonHloOutputCallbacks_MatchAndEviction) {
  const char* hlo_text = R"(
HloModule TestModule
...
ENTRY main {
  ...
  auto expected_literals = std::make_shared<ExpectedLiteralsMap>();
  (*expected_literals)["add"] =
      std::make_shared<Literal>(LiteralUtil::CreateR0<float>(7.0f));
  ...
  std::vector<HloOutputCallback> callbacks = CreateComparisonHloOutputCallbacks(
      test_module_clone.get(), ref_groups, expected_literals, *module, options,
      result_mutex, &test_result);

  bool found_add_callback = false;
  for (const auto& cb : callbacks) {
    Literal actual_literal = LiteralUtil::CreateR0<float>(7.0f);
    std::vector<std::shared_ptr<const Literal>> lits = {
        std::make_shared<Literal>(std::move(actual_literal))};
    cb.callback(/*replica_id=*/0, /*partition_id=*/0, lits);
    found_add_callback = true;
  }
  EXPECT_TRUE(found_add_callback);

  // Eviction verification: "add" must be erased from expected_literals
  EXPECT_FALSE(expected_literals->contains("add"));
  EXPECT_EQ(test_result.numeric_checks_size(), 0);
}

TEST_F(HloIsolationTest, TestComparisonHloOutputCallbacks_MismatchDetection) {
  const char* hlo_text = R"(
HloModule TestModule
...
ENTRY main {
  ...
  ::testing::TestPartResultArray failures;
  {
    ::testing::ScopedFakeTestPartResultReporter reporter(
        ...
        &failures);
    for (const auto& cb : callbacks) {
      Literal actual_literal = LiteralUtil::CreateR0<float>(100.0f);
      std::vector<std::shared_ptr<const Literal>> lits = {
          std::make_shared<Literal>(std::move(actual_literal))};
      cb.callback(/*replica_id=*/0, /*partition_id=*/0, lits);
    }
...
TEST_F(HloIsolationTest, TestComparisonHloOutputCallbacks_IncompatibleShape) {
  const char* hlo_text = R"(
HloModule TestModule
...
ENTRY main {
  ...
  auto expected_literals = std::make_shared<ExpectedLiteralsMap>();
  // Incompatible shape: f32[4] instead of f32[2,2]
  ...
  // Incompatible shape logs warning and returns early without recording
  // mismatch
  EXPECT_EQ(test_result.numeric_checks_size(), 0);
}

TEST_F(HloIsolationTest, TestComparisonHloOutputCallbacks_EdgeCases) {
  const char* hlo_text = R"(
HloModule TestModule
...
ENTRY main {
  ...
  // 1. Missing expected literal in map (empty map)
  auto empty_expected_literals = std::make_shared<ExpectedLiteralsMap>();
  std::unique_ptr<HloModule> clone1 = module->Clone();
  std::vector<HloOutputCallback> callbacks1 =
      CreateComparisonHloOutputCallbacks(clone1.get(), ref_groups,
                                         empty_expected_literals, *module,
                                         options, result_mutex, &test_result);

  Literal actual1 = LiteralUtil::CreateR0<float>(1.0f);
  std::vector<std::shared_ptr<const Literal>> lits1 = {
      std::make_shared<Literal>(std::move(actual1))};
  callbacks1[0].callback(/*replica_id=*/0, /*partition_id=*/0, lits1);
  EXPECT_EQ(test_result.numeric_checks_size(), 0);

  // 2. Null expected_literals map
  std::unique_ptr<HloModule> clone2 = module->Clone();
  std::vector<HloOutputCallback> callbacks2 =
      CreateComparisonHloOutputCallbacks(clone2.get(), ref_groups,
                                         /*expected_literals=*/nullptr, *module,
                                         options, result_mutex, &test_result);

  Literal actual2 = LiteralUtil::CreateR0<float>(1.0f);
  std::vector<std::shared_ptr<const Literal>> lits2 = {
      std::make_shared<Literal>(std::move(actual2))};
  callbacks2[0].callback(/*replica_id=*/0, /*partition_id=*/0, lits2);
  EXPECT_EQ(test_result.numeric_checks_size(), 0);

  // 3. Null or empty literals in callback
  std::vector<std::shared_ptr<const Literal>> empty_lits;
  callbacks2[0].callback(/*replica_id=*/0, /*partition_id=*/0, empty_lits);

  std::vector<std::shared_ptr<const Literal>> null_lits = {nullptr};
  callbacks2[0].callback(/*replica_id=*/0, /*partition_id=*/0, null_lits);
}
